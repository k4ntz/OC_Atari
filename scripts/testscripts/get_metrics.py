...
env = OCAtari(game_name, mode=MODE, hud=HUD,
              render_mode='rgb_array', obs_mode='dqn')
...
for agent, meth_name in zip(agents, ["Random", "DQN", "C51"]):
    if agent is not None:
        ...
        with tqdm(total=NB_SAMPLES) as pbar:
            ...
            for i in range(20*NB_SAMPLES):
                ...
                if i % 20 == 0:
                    if ONLYBOTHDEFINEDOBJECTS:
                        robjects = [
                            e for e in env.objects if e.__class__.__name__ in classes_in_both]
                        vobjects = [
                            e for e in env.objects_v if e.__class__.__name__ in classes_in_both]
                    else:
        ...
        dfs = []
        dfs.append(pd.DataFrame.from_dict(format_values(
            det_scores.cat_precisions), orient="index", columns=["Precision"]))
        dfs.append(pd.DataFrame.from_dict(format_values(
            det_scores.cat_recalls), orient="index", columns=["Recall"]))
        dfs.append(pd.DataFrame.from_dict(format_values(
            det_scores.cat_f_scores), orient="index", columns=["F-score"]))
        dfs.append(pd.DataFrame.from_dict(
            format_values(ious), orient="index", columns=["IOU"]))
        df_finals.append(
        ...
        df_finals.append(pd.DataFrame(
            columns=['Precision', 'Recall', 'F-score', 'IOU']))
...
for det_scores, agent in zip(detections_scores, agents):
    if agent is not None:
        base_dfs.append(pd.DataFrame.from_dict(
            det_scores.dict_summary, orient="index", columns=[opts.game]).T * 100)
    else:
