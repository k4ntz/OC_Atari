...
def create_bitmats(ram_saves):
    ...
    for f in range(n_frames):
        ...
        for i in range(ram_width):
            ...
            bit_array = np.array([int(d) for d in bin_string.zfill(bit_depth)])  # create bit vector of the ram value for the current position
...
def bit_correlation(bit_mats, tresh):
    ...
    plt.colorbar(mappable=img, ax=ax, orientation='vertical', fraction=.1, label='Correlation')
...
def plot_bit_activity(bit_mats_condensed, title):
    ...
    ax.set_title(title)
    plt.colorbar(mappable=img, ax=ax, orientation='vertical', fraction=.1, label='Switch Frequency')
...
def compare_bit_mats(obj_saves, no_obj_saves):
    ...
    hist_mat_no_obj = np.sum(np.abs(d_no_object_mats), axis=0) / d_no_object_mats.shape[0]
    ...
    titles = ("Tracked object absent", "Tracked object present", "Masked out inactive bits in 'absent' condition")
    ...
    for row, data, title in zip(axs, data, titles):
        ...
        row.set_title(title)
        plt.colorbar(mappable=img, ax=row, orientation='vertical', fraction=.1, label='Switch Frequency')
    
    fig.suptitle(f"Comparison of bitwise activity with respect to {opts.tracked_objects}", fontsize=20)
    fig.tight_layout()
...
class IndexTracker:
    def __init__(self, axs, bit_mats, game_imgs):
        ...
        a = axs['BITMAP']
        self.im2 = a.imshow(self.bit_mats[self.index, :, :], alpha=0.5, aspect='auto', cmap='binary', interpolation='none', vmin=0, vmax=1)
        self.im = a.imshow(self.dBits[self.index, :, :], alpha=0.75, aspect='auto', cmap='PiYG', interpolation='none', vmin=-1, vmax=1)
        a.set_xticks(np.arange(0, len(self.ram_labels), 1.0), labels=self.ram_labels)
        a.tick_params(axis='x', labelrotation=90, labelsize=6)
        plt.colorbar(mappable=self.im, ax=a, orientation='horizontal', fraction=.025)
        ...
        d = axs['DIFF_IMG']
        self.im5 = d.imshow(self.game_imgs[self.index + 1, :, :, :] - self.game_imgs[self.index, :, :, :])
    ...
    def update(self):
        ...
        self.im5.set_data(self.game_imgs[self.index + 1, :, :, :] - self.game_imgs[self.index, :, :, :])
...
fig2, ax2 = plot_bit_activity(bit_mats['all']['bin'], "Bit-Activity Across All Extracted RAM-States")
