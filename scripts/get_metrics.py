...
df_finals = []
for agent, meth_name in zip([random_agent, dqn_agent, c51_agent], ["Random", "DQN", "C51"]):
    ALL_STATS = {
             "mean_ious": [],
             "per_class_ious": {},
             "only_in_ram": {},
...
csv_report_file = "reports/all_games_report.csv"
already = None
if os.path.exists(csv_report_file):
    already = pd.read_csv(csv_report_file, index_col="Game")

stats = pd.DataFrame.from_dict(det_scores.dict_summary, orient="index", columns=[opts.game]).T * 100
if already is not None:
    stats = pd.concat([already, stats]).sort_index()

stats.to_csv(csv_report_file, index_label="Game")
print(f"Updated {csv_report_file}")