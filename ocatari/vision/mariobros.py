...
def _detect_objects(objects, obs, hud=False):
    ...
    power_block_bb = find_mc_objects(obs, objects_colors['pow_block'], size=(16,7), tol_s=2, all_colors=False)
    if power_block_bb:
    ...
    for i in objects_colors['pests']:
        pests_bb.extend([list(bb) + objects_colors['pests'][i] for bb in find_objects(obs, objects_colors['pests'][i], size=(9, 14), tol_s=5)])
    ...
    for pos in coin_positions:
        bonus_coins_bb.extend([list(bb) for bb in find_objects(obs, objects_colors['bonus_coin'], size=(9,13), tol_s=(4,4), minx=pos[0], miny=pos[1], maxx=pos[2], maxy=pos[3])])
