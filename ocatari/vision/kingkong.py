...
def _detect_objects(objects, obs, hud=False):
    ...
    player = objects[0]
    player_bb = find_mc_objects(obs, object_colors['player'], size=(8,20), maxy=260)
    if player_bb:
        player.xywh = player_bb[0]


    enemy = objects[1]
    enemy_bb = find_objects(obs, object_colors['enemy'], size=(15, 33), maxy=260)
    if enemy_bb:
        enemy.xywh = enemy_bb[0]

    girlfriend = objects[2]
    girlfriend_bb = find_mc_objects(obs, object_colors['girlfriend'], size=(6, 17), all_colors=True, tol_s=2)
    if girlfriend_bb:
    ...
    for i in object_colors['bombs']:
        bombs_bb.extend([list(bb) + object_colors['bombs'][i] for bb in find_mc_objects(obs, object_colors['bombs'][i], maxy=260)])
