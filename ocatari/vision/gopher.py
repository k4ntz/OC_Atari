...
player_colors = [[72, 160, 72], [214, 92, 92], [183, 194, 95],
...
def _detect_objects(objects, obs, hud=False):
    ...
    carrots = find_mc_objects(obs, carrot_colors, size=(
        12, 27), miny=125, tol_s=4, closing_dist=2)
    for c in carrots:
        objects.append(Carrot(*c))

    gopher = find_objects(
        obs, objects_colors["gopher"], miny=130, size=(14, 10), tol_s=2)
    for g in gopher:
        objects.append(Gopher(*g))

    blocks = find_rectangle_objects(
        obs, objects_colors["blocks"], max_size=(8, 6), maxy=182, miny=176)
    for b in blocks:
        objects.append(Empty_block(*b))

    blocks = find_rectangle_objects(
        obs, objects_colors["blocks"], max_size=(8, 6), maxy=176, miny=168)
    for b in blocks:
        objects.append(Empty_block(*b))

    blocks = find_rectangle_objects(
        obs, objects_colors["blocks"], max_size=(8, 6), maxy=168, miny=162)
    for b in blocks:
        objects.append(Empty_block(*b))

    blocks = find_rectangle_objects(
        obs, objects_colors["blocks"], max_size=(8, 12), miny=182)
    for b in blocks:
        objects.append(Empty_block(*b))

    if hud:
        birds = find_mc_objects(obs, birdcolors, size=(
            15, 18), maxy=90, tol_s=3, closing_dist=2)
        for b in birds:
            objects.append(Bird(*b))
        score = find_objects(obs, hud_color, maxy=60, size=(
            5, 9), tol_s=2, closing_active=False)
        for s in score:
