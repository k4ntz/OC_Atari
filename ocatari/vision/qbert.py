...
cubes_colors = [[45, 87, 176], [210, 210, 64],
                [192, 192, 192], [111, 111, 111]]
...
def _detect_objects(objects, obs, hud=True):
    ...
    player = find_objects(
        obs, objects_colors["player"], closing_dist=5, min_distance=1, size=(8, 20), tol_s=5)
    ...
    purple_ball = find_objects(
        obs, objects_colors["purple_ball"], min_distance=18, size=(7, 7), tol_s=3)
    for bb in purple_ball:
        objects.append(PurpleBall(*bb))

    green_ball = find_objects(
        obs, objects_colors["green_ball"], min_distance=1, size=(7, 7), tol_s=3)
    ...
    coily = find_objects(
        obs, objects_colors["coily"], min_distance=1, size=(8, 18), tol_s=5)
    for bb in coily:
        objects.append(Coily(*bb))

    sam = find_objects(
        obs, objects_colors["sam"], min_distance=1, size=(8, 18), tol_s=2)
    for bb in sam:
        objects.append(Sam(*bb))

    if hud:
        score = find_objects(
            obs, objects_colors["score"], min_distance=1, maxy=13, closing_dist=7)
        for bb in score:
            objects.append(Score(*bb))

        lives = find_objects(
            obs, objects_colors["lives"], min_distance=1, miny=14, maxy=30)
        for bb in lives:
