...
def _detect_objects(objects, obs, hud=False):
    ...
    trees_bb = []
    for col in trees_c:
        trees_bb.extend([list(bb) + [col] for bb in find_objects(obs, col, closing_active=False, closing_dist=12)])
    match_objects(objects, trees_bb, 1, 4, Tree)
    moguls_bb = []
    for col in moguls_c:
        moguls_bb.extend([list(bb) + [col] for bb in find_objects(obs, col, closing_active=False, closing_dist=12)])
    match_objects(objects, moguls_bb, 5, 3, Mogul)
    flags_bb = []
    for col in flag_c:
        flags_bb.extend([list(bb) + [col] for bb in find_objects(obs, col, closing_active=False, closing_dist=12)])
    match_objects(objects, flags_bb, 8, 4, Flag)
    if hud:
        score, clock = objects[-2:]
        score_bb = find_objects(obs, (0, 0, 0), miny=4, maxy=14, minx=50, maxx=100, closing_active=True, closing_dist=5)[0]
        score.xywh = score_bb
        clock_bb = find_objects(obs, (0, 0, 0), miny=15, maxy=25, minx=50, maxx=100, closing_active=True, closing_dist=5)[0]
        clock.xywh = clock_bb
