...
def _detect_objects(objects, obs, hud=False):
    ...
    moguls_bb = []
    for col in moguls_c:
        moguls_bb.extend([list(bb) + [col] for bb in find_objects(obs, col, closing_active=True, closing_dist=12)])
    match_objects(objects, moguls_bb, 5, 3, Mogul)
    flags_bb = []
    for col in flag_c:
        flags_bb.extend([list(bb) + [col] for bb in find_objects(obs, col, closing_active=True, closing_dist=12)])
