...
def _detect_objects(objects, obs, hud=True):
    ...
    for i in objects_colors["player"]:
        player = find_objects(
            obs, objects_colors["player"][i], min_distance=1, maxy=181, miny=165)
        for bb in player:
            p = Player(*bb)
            p.rgb = objects_colors["player"][i]
            objects.append(p)

    player_projectile = find_objects(
        obs, objects_colors["player_projectile"], min_distance=1)
    for bb in player_projectile:
        objects.append(Player_Projectile(*bb))

    for i in range(3):
        torpedos = find_objects(
            obs, objects_colors["torpedos"], min_distance=1, size=(i*2+1, i*2+1), tol_s=1)
        for bb in torpedos:
    ...
    rejuvenator = find_objects(
        obs, objects_colors["rejuvenator"], min_distance=1, size=(3, 4), tol_s=3)
    ...
    enemy_projectile = find_objects(
        obs, objects_colors["enemy_projectile"]["1"], min_distance=1, size=(4, 2), tol_s=1)
    ...
    enemy_projectile = find_objects(
        obs, objects_colors["enemy_projectile"]["2"], min_distance=1, size=(2, 6), tol_s=1)
    for bb in enemy_projectile:
        p = Enemy_Projectile(*bb)
        p.rgb = objects_colors["enemy_projectile"][i]
        objects.append(p)
    ...
    if hud:
        hud_o = find_objects(
            obs, objects_colors["hud"], min_distance=1, closing_dist=5)
        for bb in hud_o:
            objects.append(HUD(*bb))

        enemy_amount = find_objects(
            obs, objects_colors["enemy_amount"], min_distance=1)
        for bb in enemy_amount:
            objects.append(Enemy_Amount(*bb))

        life = find_objects(
            obs, objects_colors["life"], min_distance=1, miny=181)
        for bb in life:
            objects.append(Life(*bb))

        torpedos_hub = find_objects(
            obs, objects_colors["torpedos_hud"], min_distance=1, maxy=100)
