...
def _detect_objects(objects, obs, hud=False):
    ...
    player = objects[0]
    player_bb = find_mc_objects(obs, playercolors, size=(8, 17), tol_s=2, closing_active= False)
    if player_bb:
        player.xywh = player_bb[0]
    start_idx = 1
    
    bird_bb = find_objects(obs, objects_colors["Bird"], closing_dist=5, size=(8,7), tol_s=2, miny = 30)
    match_objects(objects,bird_bb,start_idx, 8,Bird)
    start_idx+=8
    
    bear_bb = find_objects(obs, objects_colors["bear"], miny=13,maxy=75,size=(14,15),tol_s=5)
    if bear_bb:
    ...
    for i,(miny,maxy) in zip(plate_per_col, floors):
        whiteplates_bb= [list(bb) + [objects_colors["WhitePlate"]] for bb in find_objects(obs, objects_colors["WhitePlate"], closing_active=False, size=(20,7), tol_s=6, miny=miny,maxy=maxy)]   
        blueplates_bb = [list(bb) + [objects_colors["BluePlate"]] for bb in find_objects(obs, objects_colors["BluePlate"], closing_active=False, size=(20,7), tol_s=6, miny=miny,maxy=maxy)]
    ...
    g_fish_bb = find_objects(obs, objects_colors["greenfish"], size=(8,6),tol_s=2)
    match_objects(objects, g_fish_bb, start_idx, 8, GreenFish)
    start_idx+=8    
    
    flag=find_objects(obs,[0,0,0],miny=43,maxy=56,minx=120,maxx=133)
    house = find_objects(obs, objects_colors["house"], minx=84, miny=42,maxx=155,maxy=62, closing_active= False)
    if len(flag)==0:
        for h in house:
    ...
    if hud:
        ...
        lifecount_bb = find_objects(obs, objects_colors["hud_objs"], closing_dist=10, minx=50, miny=19, maxx=75,maxy=32)
        if lifecount_bb:
        ...
        degrees_bb = find_objects(obs, objects_colors["hud_objs"], closing_dist=10, minx=19, miny=19, maxx=37,maxy=30)
        match_objects(objects, degrees_bb,start_idx,1,Degree)
        start_idx+=1
            
        score_bb = find_objects(obs, objects_colors["hud_objs"], closing_dist=10, minx=40, miny=8, maxx=75,maxy=18)
