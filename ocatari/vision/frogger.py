...
def _detect_objects(objects, obs, hud=False):
    ...
    #Detect lady frog
    lady_bb = find_objects(obs, objects_colors["lady frog"], maxy= 100, size =(8,11))
    match_objects(objects, lady_bb, start_idx, 1, LadyFrog)
    start_idx+=1

    flys_bb =[]
    heads_bb = []
    #Detect Alligator's Head and flys
    heads_bb = find_objects(obs, objects_colors["alligator's head"], miny= 15, maxy= 30, minx = 11, maxx =149 , size =(8,10), tol_s= 2)
    for bb in heads_bb:
        if not (obs[bb[1]+8][bb[0]] == [110,156,66]).all(): # check if it is a fly 
            flys_bb.append(bb)
            heads_bb.remove(bb)  
    match_objects(objects, heads_bb, start_idx, 1, AlligatorHead)
    start_idx+=1
    
    # Detect fly
    match_objects(objects, flys_bb, start_idx, 1, Fly)
    start_idx+=1
    
    #Detect Happy Frogs
    for i,(minx, maxx) in zip(max_frog_per_col, frog_limit_x):
        frog_bb = [bb for bb in find_objects(obs, objects_colors["happy frog"], maxy=26, miny=14, minx=minx, maxx=maxx,size=(8,10), tol_s = 2)]
        match_objects(objects, frog_bb, start_idx, i, HappyFrog)        
        start_idx += i  
    ...
    for i,(miny, maxy) in zip(max_snake_per_line, snakes_limit_y):
        snake_bb = [bb for bb in find_objects(obs, objects_colors["snake"], minx=8, maxx=152, miny=miny, maxy=maxy,size=(16,5), tol_s = 2)]
        match_objects(objects, snake_bb, start_idx, i, Snake)        
        start_idx += i  
