...
def _detect_objects(objects, obs, hud=False):
    ...
    # Player
    player_bb = find_objects(obs, object_colors["player"], miny=50, size=(8, 4), tol_s=1)
    ...
    # Player Score & Lives
    if hud:
        player_score_bb = find_objects(obs, object_colors["player_score"], maxy=25, closing_dist=10)
        match_objects(objects, player_score_bb, 7, 1, PlayerScore)

        lives_bb = find_objects(obs, object_colors["player"], miny=20, maxy=30, closing_dist=16)
