...
def _detect_objects(objects, obs, hud=False):
    ...
    #Shopping Carts
    carts_bb = find_mc_objects(obs,objects_colors["Shopping carts"], size = (8,10), tol_s = 0, closing_active= False)
    match_objects(objects, carts_bb,start_idx, 4, Cart)
    start_idx+=4

    #Biplanes
    biplanes_bb = find_objects(obs,objects_colors["Biplane"], size =(8,5), tol_s=1, closing_active= False)
    if biplanes_bb:
    ...
    if hud:
        #Score
        score_bb = find_objects(obs,objects_colors["Score"], maxy = 40, maxx=70, closing_dist=5)
        objects[start_idx] = Score(*score_bb[0]) if score_bb else NoObject()
        start_idx+=1

        #Bonus Kops
        bonus_bb = find_objects(obs,objects_colors["Bonus Kops"],minx=10, maxx = 50, miny = 40, maxy = 50)
        if bonus_bb:
            objects[start_idx] = BonusKops(*bonus_bb[0])
        start_idx+=1

        timer_bb = find_objects(obs,objects_colors["Timer"],minx=50, maxx = 75, miny = 40, maxy = 51, closing_dist=5)
        if timer_bb:
