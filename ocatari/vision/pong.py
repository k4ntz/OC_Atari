...
def _detect_objects(objects, obs, hud=False):
    ...
    player_bb = find_objects(obs, objects_colors["player"], min_distance=1, miny=30)
    if player_bb:   
        player.xywh = player_bb[0]
    ball_bb = find_objects(obs, objects_colors["ball"], min_distance=None, miny=34, maxy=194)
    if ball_bb:
        ball.xywh = ball_bb[0]
    if hud:
        player_score, enemy_score = objects[3:5]
        player_score_bb = find_objects(obs, objects_colors["player"], closing_dist=16, closing_active=True, maxy=30)
        if player_score_bb:
            player_score.xywh = player_score_bb[0]
        enemy_score_bb = find_objects(obs, objects_colors["enemy"], closing_dist=10, closing_active=True, maxy=30)
        if enemy_score_bb:
